...
# WHOLE VALUES ----
# Round Q matrix to whole numbers (1 or 0)
QUBO.Qmat.wv <- cbind(QUBO.Qmat[,1:3],round(QUBO.Qmat[,4:5]+13))
colnames(QUBO.Qmat.wv) <- c("lat","long","popNames", "A", "B")
# Generate a plot of points
plot(QUBO.Qmat.wv[,2],QUBO.Qmat.wv[,1], col=QUBO.Qmat.wv[,4], pch=20,
     xlab="Longitude", ylab="Latitude", main="QUBO sample locations (whole values)")
# Add legend
legend(x=-86.3, y=33.6,legend=c("Cluster 1", "Cluster 2"), col=unique(QUBO.Qmat.wv[,4]), pch = c(20,20))
...
clusterColors <- rbPal(1000)[as.numeric(cut(QUBO.Qmat$V6,breaks = 1000))]
QUBO.Qmat.cg <- cbind(QUBO.Qmat, clusterColors)

# Generate a plot of points
plot(QUBO.Qmat.cg$Longitude, QUBO.Qmat.cg$Latitude, col=QUBO.Qmat.cg$clusterColors, pch=16,
     xlab="Longitude", ylab="Latitude", main="QUBO sample locations")
# Add legend
legend(x=-86.3, y=33.6,legend=c("Cluster 1", "Cluster 2"), col=c("red","blue"), pch = c(20,20))
