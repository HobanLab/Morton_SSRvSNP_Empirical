...
get.allele.cat <- function(genpop, n_ind_p_pop){
  #--Set up categories for GLOBAL ALLELES
  allele_freqs<-colSums(genpop@tab, na.rm = TRUE)/(sum(n_ind_p_pop)*2)
...
# Vector of results (although no longer "by garden")
...
# Assign names to values, and write to a file
