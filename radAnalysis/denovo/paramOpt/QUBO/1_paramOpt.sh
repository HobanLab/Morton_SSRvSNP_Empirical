...
# Script for looping over the denovo.params file, detailing the parameter space for the Stacks de novo assembly
...
while IFS=, read -r m M n gtalpha; do
	...
	mkdir ./output/QUBO_m${m}_M${M}_n${n}_ga${gtalpha}
	...
	denovo_map.pl -T 28 -m $m -M $M -n $n --gt-alpha $gtalpha --samples ../../process_RADtags/QUBO/ --popmap ./popmap_QUBO_paramOpt -o ./output/QUAC_m${m}_M${M}_n${n}_ga${gtalpha}
	sleep 5s
done < ../denovo.params
echo "All QUBO parameter optimization runs complete!"
